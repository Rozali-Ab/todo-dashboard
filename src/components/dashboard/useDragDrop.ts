...
export const useDragDrop = () => {
	...
	const onDragStart = (evt: DragEvent) => {
		...
		// evt.target.clie; взять высоту таски
		const taskHeight = evt.target?.offsetHeight < 300 ?  300 : evt.target.offsetHeight;

		document.documentElement.style.setProperty('--current-drag-task-height', `${taskHeight}px`);
		if (taskId) {
